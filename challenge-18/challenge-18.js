...
function correctCPF(cpf){
    return cpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/g, function( regex, $1, $2, $3,$4 ){
...
return tag.replace(/(<(\w+)>)([\wáàâãéèêíïóôõöúçñ\s]+)(<\/\w+>)/ig, function(regex, $1, $2, $3, $4){

    return $1 + 'O texto dentro da tag "' + $2 + '" é "' + $3 + '"' + $4 + '\n';

});

}

console.log(showTags('<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'));
